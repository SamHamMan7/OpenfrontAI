...
async function createLobby(numWorkers: number): Promise<{ gameID: string; widx: number }> {
  ...
  for (let i = 0; i < numWorkers; i++) {
    try {
      ...
      console.error(`[BOT] Could not fetch w${i} create_game. Is the server running?`, (e as Error).message);
    }
  }
          ...
          if (!gameStarted) {
            ...
            if (isCreator && clients.length >= 2) {
              ...
              try {
                ...
                console.error(`[BOT] Could not start game. Server may be offline.`, (e as Error).message);
              }
            }
          }
          break;
